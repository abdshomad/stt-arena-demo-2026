"""NVIDIA Canary 1B Flash — real NeMo model nvidia/canary-1b-flash."""
...
def load(variant: str):
    ...
    device = torch_device()
    model = EncDecMultiTaskModel.from_pretrained("nvidia/canary-1b-flash", map_location=device)
    if device == "cuda":
        model = model.cuda()
    return model
...
app = create_app(
    engine_id="nvidia-canary-1b-flash",
    load=load,
