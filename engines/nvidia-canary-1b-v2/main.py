"""NVIDIA Canary 1B v2 — real NeMo model nvidia/canary-1b-v2."""
...
def load(variant: str):
    ...
    device = torch_device()
    model = EncDecMultiTaskModel.from_pretrained("nvidia/canary-1b-v2", map_location=device)
    if device == "cuda":
        model = model.cuda()
    return model
